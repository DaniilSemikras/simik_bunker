...
function normalizeHealthState(rawState, fallbackValue = "") {
    ...
    return {
        diseaseName: String(rawState.diseaseName || parseHealthState(fallbackValue, maxStage).diseaseName || (stage > 0 ? "Заболевание" : "")).trim(),
        stage,
        maxStage,
        status: terminal ? "Смертельно болен" : stage === 0 ? "Полностью здоров" : "Болен",
        terminal
...
function randomizeGenderAge(value, random = Math.random) {
    const gender = String(value || "")
        ...
        .replace(/[\s,;—-]+$/g, "")
        .trim() || "Не указан";
    const age = 18 + Math.floor(Math.max(0, Math.min(0.999999999, Number(random()) || 0)) * 103);
